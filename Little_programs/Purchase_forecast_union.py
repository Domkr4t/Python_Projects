...
def handle_beer(nomenclature, stock, total_quantity):
    ...
    elif nomenclature == "Амбирлэнд Вишнёвый крик":
...
def calc_end_date_merka():
    ...
    if day_of_week == 0:
        if selected_store == "10_Коломна_Советская5" or selected_store == "18_Коломна_Кирова38а":
...
def calc_end_date_beer():
    ...
    if day_of_week == 0:
        ...
        if selected_store == "7_Балашиха_Свердлова25" or selected_store == "17_Балашиха_Маяковского12с2":
            return ((start_date_order + timedelta(days=3))).replace(hour=23, minute=59, second=59, microsecond=59)
        else:
...
def text_for_shop():
    ...
    elif selected_store == "18_Коломна_Кирова38а":
        return ["Добрый день.", "Заказ ИП Петрова,", "Г. Коломна Проспект Кирова 38а"]
    elif selected_store == "17_Балашиха_Маяковского12с2":
...
def generate_report():
    ...
    for index, row in residuals.iterrows():
        ...
        if start_date_str != "01.01.1900 00:00:00" or end_date_str != "31.12.9999 23:59:59":
            ...
            sales_data_filtered_date = sales_data_filtered_shop
        ...
        if not matching_rows.empty and matching_rows["Категория товара"].str.contains("Амбирлэнд").any() | matching_rows["Номенклатура"].str.contains("Квас").any():
            ...
            # Замена именований
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Жигулёвское", "Жигулёвское Ф")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Светлое НФ", "Боровское светлое НФ")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Тёмное", "Боровское тёмное Ф")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Пилснер Ф", "Пилснер Ф")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Пилснер НФ", "Пилснер НФ")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Светлое Ф", "Бундес Ф")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Пшеничное", "Вайс канцлер НФ")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Вишнёвый крик", "Вайлд Черри")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Хеллес", "Боровское урожайное")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Крепкое", "Империал канцлер НФ")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Лёгкое", "Домашнее")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд IPA", "Хмельзилла ИПА НФ")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд Грейпфрутовый эль", "Леди на велосипеде")
            beer_results["Номенклатура"] = beer_results["Номенклатура"].replace("Амбирлэнд APA", "Бирконг НФ АРА")
    ...
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Жигулёвское Ф", "Амбирлэнд Жигулёвское")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Боровское светлое НФ", "Амбирлэнд Светлое НФ")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Боровское тёмное Ф", "Амбирлэнд Тёмное")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Пилснер Ф", "Амбирлэнд Пилснер Ф")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Пилснер НФ", "Амбирлэнд Пилснер НФ")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Бундес Ф", "Амбирлэнд Светлое Ф")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Вайс канцлер НФ", "Амбирлэнд Пшеничное")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Вайлд Черри", "Амбирлэнд Вишнёвый крик")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Боровское урожайное", "Амбирлэнд Хеллес")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Империал канцлер НФ", "Амбирлэнд Крепкое")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Домашнее", "Амбирлэнд Лёгкое")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Хмельзилла ИПА НФ", "Амбирлэнд IPA")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Леди на велосипеде", "Амбирлэнд Грейпфрутовый эль")
    beer_second_table_our_name["Номенклатура(Воронеж)"] = beer_second_table_our_name["Номенклатура(Воронеж)"].replace("Бирконг НФ АРА", "Амбирлэнд APA")
